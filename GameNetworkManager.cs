...
public class GameNetworkManager : MonoBehaviour
{
    ...
    private void SteamMatchmaking_OnLobbyCreated(Result _result, Lobby _lobby)
    {
        ...
        {
            Debug.Log($"lobby was not created: {_result}", this);
            return;
        }
        _lobby.SetPublic();
        _lobby.SetJoinable(true);
        _lobby.SetData("Name", _lobby.Owner.Name);
        _lobby.SetData("Id", _lobby.Owner.Id.ToString());
        _lobby.SetGameServer(_lobby.Owner.Id);
        Debug.Log($"lobby created {_lobby.Owner.Name}");
        NetworkTransmission.instance.AddMeToDictionaryServerRPC(SteamClient.SteamId, SteamClient.Name, NetworkManager.Singleton.LocalClientId,false);  //
    }
    ...
    public void Disconnect()
    {
        ...
        if (!GameManager.instance.matchEndScreen.activeInHierarchy)
        {
    ...
    private void Singleton_OnClientConnectedCallback(ulong _cliendId)
    {
        ...
        GameManager.instance.JoiningGame();
        NetworkTransmission.instance.AddMeToDictionaryServerRPC(SteamClient.SteamId, SteamClient.Name, _cliendId,false);
